    ...
    <div ref={cardViewportRef} className="mx-auto w-full max-w-3xl" style={{ perspective: '1800px', height: `${357 * cardScale}px` }}>
      <div ref={cardFrameRef} role="button" tabIndex={0} onClick={() => setFlipped((value) => !value)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') setFlipped((value) => !value); }} className="relative h-[357px] w-[768px] origin-top-left cursor-pointer transition-transform duration-700" style={{ transformStyle: 'preserve-3d', transform: `${flipped ? 'rotateY(180deg)' : 'rotateY(0deg)'} scale(${cardScale})` }}>
        <div ref={frontRef} className="absolute inset-0 overflow-hidden rounded-2xl border border-[#9A650B] bg-gradient-to-br from-[#FFF9DA] via-[#EEC550] to-[#B87910] p-5 text-[#0B3C5D] shadow-xl" style={{ backfaceVisibility: 'hidden' }}>
          ...
          <div className="relative flex items-center justify-between gap-4"><img src="/mandir-logo.png" alt="Jagannath Mandir Noida" className="h-12 w-[112px] object-contain object-left" /><div className="text-right"><p className="font-serif text-base font-bold tracking-wide">Shree Swarna Khetra</p><p className="text-[9px] font-bold uppercase tracking-[0.2em] text-[#0B3C5D]/75">Jagannath Mandir, Noida</p></div></div>
          ...
          <div className="relative mt-3 flex items-center justify-between gap-3 border-t border-[#0B3C5D]/20 pt-2"><span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-700 bg-emerald-100 px-2.5 py-1 text-[9px] font-extrabold tracking-wide text-emerald-800 shadow-sm"><ShieldCheck className="h-3.5 w-3.5" /> ACTIVE &amp; VERIFIED</span><div className="flex items-center gap-2 text-right"><img src="/svslogo.png" alt="SVS Logo" className="h-8 w-8 rounded-full border border-[#0B3C5D]/15 bg-white object-contain p-0.5 shadow-sm" /><div className="leading-tight"><p className="text-[8px] font-medium uppercase tracking-[0.16em] text-[#0B3C5D]/65">Issued by</p><p className="mt-0.5 text-[10px] font-bold">Samudayik Vikas Samiti</p></div></div></div>
        </div>
